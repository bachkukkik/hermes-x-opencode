...
seed_righthand_man() {
    ...
    # Idempotency: if the profile already has its SOUL.md, we've seeded before.
    if [ -f "${target}/SOUL.md" ]; then
        echo "== righthand-man profile already seeded, skipping."
        return 0
    fi

    # The hermes CLI lives in the WebUI venv and is not on PATH. Without it we
    # cannot clone the active profile; warn and defer to the next boot.
    if [ ! -x /app/venv/bin/hermes ]; then
        echo "!! /app/venv/bin/hermes not found; cannot clone righthand-man profile. Skipping (will retry next boot)."
        return 0
    fi

    echo "== Seeding righthand-man profile..."

    # Clone the active (default) profile into a new righthand-man profile so we
    # inherit config.yaml, .env, skills/, etc. Run as hermeswebui for ownership.
    # --no-alias avoids interactive wrapper-script prompts during the clone.
    ...
    # Overwrite the cloned SOUL.md with the curated orchestrator doctrine. The
    # content is embedded here via a quoted heredoc so the seed is self-contained
    # at runtime — it has no dependency on the build tree (which is NOT copied
    # into the image; only the lib scripts land at /usr/local/bin/lib/).
...
SOULEOF
    ...
    echo "== Seeded righthand-man profile at ${target}"
}

...
| `start_opencode_serve()` | `lib/service-opencode.sh` | Starts OpenCode serve as `hermeswebui` via `su`. Creates `~/.local/state` and chowns it before starting (EACCES fix — opencode serve writes state files there). Skips if `opencode` binary not found. |

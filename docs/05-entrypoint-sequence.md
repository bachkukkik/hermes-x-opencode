...
├── service-opencode.sh        (29 lines — OpenCode serve service startup; mkdir ~/.local/state + chown before serve)
...
```
 ...
 5. generate_config()            — writes config.yaml with multi-model models dict, delegation.model (if HERMES_DELEGATION_MODEL set), delegation.provider (if HERMES_DELEGATION_PROVIDER set)
 6. generate_opencode_config()   — writes opencode.jsonc with plugins, permissions, discovered models; chowns to hermeswebui
 6b. copy root config (fix #28)  — copies opencode.jsonc to /root/.config/opencode/ so root sees providers (idempotent: readlink -f guard skips if source and dest resolve to same path)
 6c. symlink session DB (fix #29) — symlinks /root/.local/share/opencode → hermeswebui's data dir
 7. validate_opencode_zen_key() — if OPENCODE_ZEN_API_KEY is set, validates it against Zen API; warns on failure (fix #30)
...
13. start_opencode_serve()       — if OPENCODE_SERVE_ENABLED=true: mkdir -p ~/.local/state + chown (EACCES fix), then su hermeswebui -c "opencode serve --port 4096 --hostname 0.0.0.0" &
                                else: log and skip
...
`generate_config()` iterates `DISCOVERED_MODELS` and writes each model ID as a key in the `custom_providers[0].models` dict with a hybrid value: known families are pinned to their true context length (`resolve_ctx_len()`), the default model always gets an explicit `context_length` (fallback 200000), and unknown families are emitted as `{}` so the hermes-agent self-resolves at runtime. The `key_env: OPENAI_API_KEY` field instructs the Hermes runtime to read the API key from the environment variable at request time. The `model` section writes both `default` and `name` keys from the resolved Hermes default (`HERMES_DEFAULT_MODEL` if set, else `OPENAI_DEFAULT_MODEL`):

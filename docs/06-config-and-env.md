...
| `OPENCODE_FALLBACK_MODEL` | No | — (unset) | Runtime fallback model for OpenCode. When set, `config-opencode.sh` adds the `opencode-runtime-fallback` plugin to `opencode.jsonc` and retries failed OpenCode LLM calls (rate limits, quota, 5xx, timeouts) against this model. Resolved with the same per-model provider routing as `OPENCODE_DEFAULT_MODEL` (bare id → litellm if OpenAI creds present, else opencode Zen). Leave unset for no fallback. |

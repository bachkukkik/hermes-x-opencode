...
- **OpenCode provider block** (issue #46): When `OPENCODE_ZEN_API_KEY` is set, an explicit `opencode` provider entry with `apiKey: {env:OPENCODE_ZEN_API_KEY}` is generated in `opencode.jsonc`. Also seeds `auth.json` as fallback credential store and passes `OPENCODE_ZEN_API_KEY` explicitly through `su` in `service-opencode.sh`. Required for built-in `opencode/` models (e.g. `deepseek-v4-flash-free`) to authenticate.
- **LiteLLM provider auth** (issue #50): `service-opencode.sh` must pass ALL provider env vars through `su` — not just `OPENCODE_ZEN_API_KEY`. Previously `OPENAI_API_KEY` and `OPENAI_BASE_URL` were stripped, causing `{env:OPENAI_API_KEY}` in the litellm provider block to resolve to empty (401 "no key passed in"). `auth.json` now seeds both `opencode` and `litellm` provider keys as fallback credentials.
